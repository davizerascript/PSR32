...
## Correção de shoulders — revisão v0.3.1

O teste físico informou uma troca dentro de cada lado do controle: L1 acionava L2, L2 acionava L1, R1 acionava R2 e R2 acionava R1. O launcher agora troca somente os rótulos semânticos SDL `leftshoulder`/`lefttrigger` e `rightshoulder`/`righttrigger`, preservando os índices físicos e a separação esquerda/direita. O fixture local confirmou `SHOULDER_SWAP_TEST=PASS`. A correção fica ativa por padrão e pode ser desativada com `PS2_SWAP_SHOULDERS=0` em uma imagem que já esteja correta.
